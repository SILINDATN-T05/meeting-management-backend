...
router.post('/process_user', bodyParser, function (req, res) {
  ...
  async.waterfall([
    ...
    function DoAuth (user, payload, next) {
      ...
      let auth = AuthDetail()
      auth.user = user._id
      auth.channel = data.platform
      auth.uuid = data.platform === 'APP' ? user.msisdn : payload.username
      auth.organizationID = payload.organizationID
      auth.status = data.platform === 'WEB-API' ? 'ACTIVE' : 'REISSUE'
      auth.STATUS = data.platform === 'WEB-API' ? 'ACTIVE' : 'REISSUE'
      auth.device = {
        deviceId: data.platform === 'APP' ? user.msisdn : payload.username,
      ...
      auth.persist(admin, function (error, auth_data) {
        if (!error && auth_data) {
          ...
          successMessge.channel = auth_data.channel
...
router.post('/reissue_user_password', bodyParser, function (req, res) {
  ...
  async.waterfall([
    function doPassword (next) {
      ...
      data['p_successText'] = 'TN Portal. Your Password is ' + p_password + ', please change immediatley.If you did not make this request,contact Silinda Co. at +27798308985.'
      data['successText'] = 'TN Portal Web-API. Your Password has been reset to ' + password + ', please change immediatley.If you did not make this request,contact Silinda Co. at +27798308985.'
    ...
    function DoAuth (user, payload, next) {
      let expiry_date = new Date()
      expiry_date.setDate(expiry_date.getDate() + 30)
      AuthDetail.findOne({user: user._id, organizationID: payload.organizationID}, function (err, auth) {
        if (!err && auth) {
          auth.status = 'REISSUE'
          auth.STATUS = 'REISSUE'
